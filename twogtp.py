...
class GTP_match:
    ...
    def endgame_contest(self, sgfbase):
        ...
        for endgamefile in self.endgamefilelist:
            ...
            if (result1 != result2):
                print(endgamefile + ":", plain_result1.group(), plain_result2.group(), "Difference:")
        ...
        for i in range(games):
            ...
            if last_streak == self.streak_length:
                if last_color == "W":
                    ...
                    if self.handicap == 1:
                        self.handicap = 2
                    print("White wins too often. Increasing handicap to %d" % (self.handicap))
                else:
                    if self.handicap > 0:
                        self.handicap -= 1
                        if self.handicap == 1:
                            self.handicap = 0
                        print("Black wins too often. Decreasing handicap to %d" % (self.handicap))
                    else:
                        self.handicap = 2
                        game.swap_players()
...
if endgame_filelist != []:
    ...
    for res in results:
        ...
        if res > 0.0:
            ...
            win_black += 1
    print("%d games, %d wins for black, %d wins for white." % (len(results), win_black, win_white))
